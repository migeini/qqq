...
void LCD_GPIO_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_9|GPIO_Pin_8|GPIO_Pin_7|GPIO_Pin_6|GPIO_Pin_5;	//CS,REST,RS,SDA,SCLK
	GPIO_InitStructure.GPIO_Mode=GPIO_Mode_Out_PP;	//推挽输出
	GPIO_InitStructure.GPIO_Speed=GPIO_Speed_50MHz;	//速度50MHZ
  GPIO_Init(GPIOB,&GPIO_InitStructure);	
	
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_10|GPIO_Pin_11|GPIO_Pin_12;	//ROM_CS,ROM_SCK,ROM_IN
	GPIO_InitStructure.GPIO_Mode=GPIO_Mode_Out_PP;	//推挽输出
	GPIO_InitStructure.GPIO_Speed=GPIO_Speed_50MHz;	//速度50MHZ
  GPIO_Init(GPIOC,&GPIO_InitStructure);	

	
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_4;	//ROM_OUT 
...
void transfer_data1(u32 data)
{
    char i;

    cs1 = 0;  // 使能设备
    rs = 1;   // 选择寄存器或数据寄存器，根据具体设备而定

    for (i = 0; i < 32; i++)  // 发送 32 位数据
    {
        sclk = 0;  // 时钟线拉低
        if (data & 0x80000000)  // 检查最高位
            sid = 1;  // 发送 1
        else
            sid = 0;  // 发送 0
        sclk = 1;  // 时钟线拉高
